...
const (
	...
	insertSQL string  = `INSERT INTO %s.%s (date, name, tags, value, timestamp) VALUES (?, ?, ?, ?, ?)`
	selectSQL string  = "SELECT COUNT() AS CNT, (intDiv(toUInt32(timestamp), %d) * %d) * 1000 as t, name, tags, quantile(%f)(value) as value FROM %s.%s"
	whereSQL  string  = " WHERE date >= toDate(%d) AND timestamp >= toDateTime(%d) AND timestamp <= toDateTime(%d) "
)
